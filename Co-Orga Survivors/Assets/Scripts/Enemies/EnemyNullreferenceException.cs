...
public class EnemyNullreferenceException : BaseUnit
{
    ...
    {
        if (collision.gameObject.name.Contains("NullReference", StringComparison.InvariantCultureIgnoreCase))
            return;

        if (collision.gameObject.name.Contains("Player", StringComparison.InvariantCultureIgnoreCase))
            Destroy(this);
